  ...
  return (
    <Card className="mb-4">
      ...
      <TouchableOpacity
        ...
        activeOpacity={0.8}
      >
        <PlusCircle size={18} stroke="#0f172a" strokeWidth={1.5} />
        <Text className="text-slate-900 font-semibold text-sm ml-2">{t("portfolio.addFunds")}</Text>
      </TouchableOpacity>
    </Card>
  );
}
    ...
    <Card className="mb-3">
      <View className={`flex-row items-center mb-3 ${isRTL ? "flex-row-reverse" : ""}`}>
        ...
        <View className={`flex-1 ${isRTL ? "mr-3 items-end" : "ml-3"}`}>
          <Text className="font-bold text-slate-900">{offer.startup_name || offer.startup_id}</Text>
    ...
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
      <View className="flex-1 bg-black/50 items-center justify-center px-6">
        <View className="bg-white rounded-2xl p-6 w-full">
          <Text className={`font-black text-slate-900 text-lg mb-4 ${isRTL ? "text-right" : "text-left"}`}>
            {t("portfolio.addFundsTitle")}
          </Text>
          <TextInput
            className="border border-slate-200 rounded-xl px-3 py-3 text-slate-900 mb-4"
